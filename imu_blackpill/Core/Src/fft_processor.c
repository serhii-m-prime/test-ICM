...
void FFT_Processor_Init(FFT_Processor *processor, bool remove_mean) {
	...
	processor->remove_mean = remove_mean;
...
uint8_t FFT_Processor_Process(FFT_Processor *processor, FFT_Result *result) {
	...
	const float32_t fft_size = (float32_t) FFT_SIZE;

	if (!processor->ready) {
		return 0;
	}

	if (processor->remove_mean) {
		float32_t mean;
		arm_mean_f32(processor->input, FFT_SIZE, &mean);
		arm_offset_f32(processor->input, -mean, processor->input, FFT_SIZE);
	}

	arm_rfft_fast_f32(&processor->handler, processor->input, processor->output, 0);
	result->magnitude[0] = fabsf(processor->output[0]) / fft_size;
	...
	/* Convert the single-sided spectrum to input-signal amplitude units. */
	arm_scale_f32(&result->magnitude[1], 2.0f / fft_size,
			&result->magnitude[1], FFT_SIZE / 2 - 1);

	arm_max_f32(
	    &result->magnitude[1], /* Exclude DC from peak detection. */
	    FFT_SIZE / 2 - 1,
	    &max_value,
	    &max_index
	);
